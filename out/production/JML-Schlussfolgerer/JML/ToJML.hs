...
isPure methods funName = case parse parseFunDefs methods of
  Nothing -> throw $ NoteExcp "{{isPure}}: parse parseFunDefs methods -> Nothing"
  Just (extDeclList,_) ->
    let getFunExtDecl :: Maybe ExternalDeclaration
        getFunExtDecl = findParsedFunction funName extDeclList
    in case getFunExtDecl of
       Nothing -> throw $ NoteExcp $ printf "{{isPure}}: searched function:%s wasn't found" funName
  ...
  where
    ...
    process3 localVariables (VarStmt a@(VarExpr _ _ _)) = process4 localVariables a
    process3 localVariables (VarStmt _) = throw $ NoteExcp "{{isPure -> process3}}: -> VarStmt (-/>VarExpr)"
    ...
    process3 _ (AssignStmt _ _) = throw $ NoteExcp "{{isPure -> process3}}: -> AssignStmt -/> AssignExpr"
    ...
    process3 localVariables (ForStmt stmtAcc cond _ stmtBody) =
      ...
      --all (process3 (localVariables ++ getCompStmtLocalVariables compStmtAcc)) forList
    ...
    --VarExpr {varType :: Maybe (Type Types), varObj :: [String], varName :: String}
    process3 localVariables (FunCallStmt (FunCallExpr (VarExpr _ _ varName) _)) = isPure methods varName
    process3 localVariables (FunCallStmt (FunCallExpr _ _)) = throw $ NoteExcp "{{isPure -> process3}}: FunCallStmt (FunCallExpr (-/>VarExpr) _)"
    process3 localVariables (FunCallStmt _) = throw $ NoteExcp "{{isPure -> process3}}: FunCallStmt (-/>FunCallExpr)"
    --TryCatchStmt {tryBody :: Statement, catchExcp :: Type Exception, catchBody :: Statement, finallyBody :: Statement}
    process3 localVariables (TryCatchStmt tryBody _ catchBody finallyBody) = False
      --all (process2 localVariables) [tryBody,catchBody,finallyBody]
    --ReturnStmt {returnS :: Maybe Expression}
    process3 localVariables (ReturnStmt Nothing) = True
    ...
    --VarExpr {varType :: Maybe (Type Types), varObj :: [String], varName :: String}
    process4 localVariables (VarExpr Nothing _ varName) = varName `elem` localVariables
    process4 localVariables (ArrayExpr (VarExpr _ _ _) Nothing) = True
    process4 localVariables (ArrayExpr (VarExpr _ _ _) (Just idx)) = process4 localVariables idx
    ...
    process4 localVariables (FunCallExpr (VarExpr _ _ varName) _) = isPure methods varName
    process4 localVariables (FunCallExpr _ _) = throw $ NoteExcp "{{isPure -> process4}}: FunCallExpr (-/> VarExpr) _"
    process4 localVariables (CondExpr eiff ethenn elsee) = all (process4 localVariables) [eiff,ethenn,elsee]
    process4 localVariables (AssignExpr (VarExpr Nothing _ varName1) assEright)
      | varName1 `notElem` localVariables = False
      | otherwise = process4 localVariables assEright
    process4 localVariables (AssignExpr (VarExpr _ _ _) assEright) = process4 localVariables assEright
    process4 localVariables (AssignExpr _ _) = throw $ NoteExcp "{{isPure -> process4}}: AssignExpr (-/> VarExpr) _"
    process4 localVariables (ExcpExpr excpName excpmsg) = False
    process4 localVariables (ReturnExpr Nothing) = True
    process4 localVariables (ReturnExpr (Just expr)) = process4 localVariables expr
    process4 _ _ = True

declarePure :: String -> String -> String
declarePure methods seeked = fst $ fromJust $ parse purify methods
  where
    purify :: Parser String
    purify = do
      state <- getState'
      parsed <- parseFunDef
      rest <- getState'
      ...
      if seeked == funName then
        let fun                 = fromJust $ takeUntilBracesClosed state '{' '}'
            ...
            purified            = fromModifiers modifiers ++ " /*@ pure @*/" ++ last splittedAtModifiers
        in return purified
      else do
        continue <- purify
        return continue

declarePureOnlyIf :: String -> String -> String
declarePureOnlyIf methods funName | isPure methods funName = declarePure methods funName
declarePureOnlyIf methods funName = getUnparsedFunction methods funName

declareAllPureOnlyIf :: String -> String
declareAllPureOnlyIf methods =
  let funsNames = getFunsNames methods
  in foldl' (\l r -> l ++ declarePureOnlyIf methods r ++ "\n\n") "" funsNames
...
getRequireBehavior called methods funName = case parse parseFunDefs methods of
  Nothing -> throw $ NoteExcp "{{getRequireBehavior}}: parse parseFunDefs methods -> Nothing"
  Just (extDeclList,_) ->
    let getFunExtDecl :: Maybe ExternalDeclaration
        getFunExtDecl = findParsedFunction (getOriginalFunName funName) extDeclList
    in case getFunExtDecl of
      Nothing -> throw $ NoteExcp $ printf "{{getRequireBehavior}}: searched function:%s wasn't found" funName
      Just lv@(FunDef _ (FunCallStmt (FunCallExpr (VarExpr _ _ _) funArgs)) _ funBody) ->
  ...
  where
    ...
    --AssignStmt {varModifier :: [Modifier], assign :: Expression}
    process1 stmts _ lv condExpr (AssignStmt _ assign) = filter (\(a,_,_) -> isJust a)$ process2 stmts lv condExpr assign
    --CondStmt {condition :: Expression, siff :: Statement, selsee :: Statement}
    ...
    process1 stmts _ lv condExpr (ReturnStmt Nothing) = [(Nothing,JMLExpr condExpr,Nothing)]
    process1 stmts funArgs lv condExpr a@(ReturnStmt (Just expr)) =
      let requires = process2 stmts lv condExpr expr
          ensures  = getEnsures methods stmts funArgs lv expr
    ...
    --VarExpr {varType :: Maybe (Type Types), varObj :: [String], varName :: String}
    process2 stmts lv condExpr a@(VarExpr (Just _) _ varName) = [(Nothing,undefined,Nothing)] --TODO: is this right?
    process2 _ lv condExpr a@(VarExpr Nothing _ varName) =
      if varName `notElem` lv then throw $ NoteExcp "{{getRequireBehavior -> process2}}: VarExpr _ varName: varName is not a local variable"
      else [(Nothing,JMLExpr condExpr,Nothing)]
    --ArrayExpr {arrName :: Expression, index :: Maybe Expression}
    process2 _ lv condExpr a@(ArrayExpr (VarExpr _ _ varName) Nothing)
      | varName `elem` lv = [(Nothing,JMLExpr condExpr,Just $ JMLExpr a)]
    ...
    --FunCallExpr {funName :: Expression, funArgs :: [Expression]}
    process2 _ lv condExpr (FunCallExpr (VarExpr _ _ varName) _) = {-throw $ NoteExcp $ printf "209:\n%s\t%s" (show called) varName-}
      let rec = getRequireBehavior True methods varName
      in --throw $ NoteExcp $ printf "211:\n%s" (show $ refineRes $ appendOriginalCondExpr rec varName condExpr)
      refineRes $ appendOriginalCondExpr rec varName condExpr
    ...
    process2 _ lv condExpr (ExcpExpr excpName _) = [(Just excpName,JMLExpr condExpr,Nothing)]
    --ReturnExpr {returnE :: Maybe Expression}
    process2 _ lv condExpr (ReturnExpr Nothing) = throw $ NoteExcp "this should be unreachable"--[(Nothing,JMLExpr condExpr,Nothing)]
    process2 _ lv condExpr (ReturnExpr (Just _)) = throw $ NoteExcp "this should be unreachable"
    {-
    process2 stmts lv condExpr (ReturnExpr (Just expr)) = 
      let ensures = getEnsures methods stmts lv expr
      in throw $ NoteExcp "MEOWWWWW"--[(Nothing,JMLExpr condExpr,ensures)]
    --ExcpExpr {excpName :: Exception, excpmsg :: Maybe String}
    -}
    {-
    process2 lv condExpr (ReturnExpr (Just expr)) = process2 lv condExpr expr
    process2 lv condExpr (ReturnExpr Nothing) = [(Nothing,JMLExpr condExpr,)]
    -}
    process2 _ _ condExpr a = [(Nothing,JMLExpr condExpr,Nothing)]
    
    assignExists :: [String] -> Expression -> ()
    --VarExpr {varType :: Maybe (Type Types), varObj :: [String], varName :: String}
    assignExists lv (VarExpr Nothing _ varName)
    ...
    appendOriginalCondExpr old funName condExpr =
      let hasFunName :: Expression -> Bool
          ...
          hasFunName (VarExpr _ _ varName) = --varName == funName
            ...
            in splitIt == funName
          hasFunName expr = False--throw $ NoteExcp $ printf "{{getRequireBehavior -> process2 -> appendOriginalCondExpr}}:\n%s" (show expr)
    ...
    refineRes list = refine' list [] where
         refine' [] res = res
         refine' ((Nothing,_,Nothing):rest) res = refine' rest res
    ...
    {-insertNormal :: Expression -> [(Maybe Exception,JMLExpr)] -> [(Maybe Exception,JMLExpr,Maybe JMLExpr)]
    insertNormal condExpr [] = [(Nothing,JMLExpr condExpr,Just $ JMLExpr BoolLiteral True)]
    insertNormal _ list = list-}
    
    checkCondition :: [String] -> Expression -> ()
    checkCondition lv expr =
      let exists list = filter (flip notElem lv) list
          throwing list = throw $ NoteExcp $ printf "{{getRequireBehavior -> process1 -> checkCondition}}: variables: %s in conditional statement are global"
    ...
    insertEnsures requires ensures = foo requires ensures [] where
      ...
      foo [] _ res = res
      foo _ [] res = res
      foo ((Nothing,b,_):rest) (ens:restt) res = foo rest restt (res ++ [(Nothing,b,ens)])
      foo (a:rest) restt res = foo rest restt (res ++ [a])
      
{-    insertEnsures requires ensures = concatMap (\case
           (Nothing,b,_) -> map (\ens -> (Nothing,b,ens)) ensures--(Nothing,b,ensures)
           a             -> [a]) requires
-}    
  ...
  -}
    ...
    attachFunName2 funName (ReturnStmt Nothing) = ReturnStmt Nothing
    ...
    attachFunName2' funName (ArrayExpr arrName Nothing) = ArrayExpr (attachFunName2' funName arrName) Nothing
    ...
    attachFunName2' funName (ReturnExpr Nothing) = ReturnExpr Nothing
...
getEnsures :: String -> [Statement] -> [String] -> [String] -> Expression -> [Maybe JMLExpr]
{-
  = 
  | ArrayExpr {arrName :: Expression, index :: Maybe Expression}
  | BinOpExpr {expr1 :: Expression, binOp :: BinOp, expr2 :: Expression}
...
-}
getEnsures methods stmts funArgs lv a = case a of
  --IntLiteral Int
  ...
  --FunCallExpr {funName :: Expression, funArgs :: [Expression]}
  FunCallExpr (VarExpr _ _ funName) funArgsList -> process1 (getRequireBehavior True methods funName) methods funName funArgsList
{-    | isPure methods funName -> process1 (getRequireBehavior True methods funName) methods funName funArgsList
    | otherwise              -> [Nothing]-} 
  --VarExpr {varType :: Maybe (Type Types), varObj :: [String], varName :: String}
  VarExpr (Just _) _ _ -> throw $ NoteExcp "{{getEnsures}}: VarExpr (Just _) _ _: why of Just?"
  a@(VarExpr Nothing _ varName)                 ->
    ...
    if varName `elem` lv && varName `notElem` funArgs then --throw $ NoteExcp $ printf "423:\n%s\t%s" varName (show lv)
      ...
      in if typ /= "For" then getEnsures methods stmts funArgs lv expr2
         else [Just $ JMLExpr $ VarExpr Nothing objs vn2]
  ...
  BinOpExpr exprL binOp exprR                   ->
    let left  = getEnsures methods stmts funArgs lv exprL
        right = getEnsures methods stmts funArgs lv exprR
    in if any isNothing (left++right) then [Nothing] -- any (all isNothing) (left:right:[])
       else let extractExprL = map (\(Just (JMLExpr l))->l) left--(filter (/=Nothing) left)
                extractExprR = map (\(Just (JMLExpr r))->r) right--(filter (/=Nothing) right)
            in zipWith (\l r -> Just $ JMLExpr $ BinOpExpr l binOp r) extractExprL extractExprR--[Just $ JMLExpr $ BinOpExpr ll binOp rr]
  --UnOpExpr {unOp :: UnOp, expr :: Expression}
  UnOpExpr unOp expr                            ->
    let extractExpr = map (\(Just (JMLExpr x))->x) $ getEnsures methods stmts funArgs lv expr
  ...
  --AssignExpr {assEleft :: Expression, assEright :: Expression}
  AssignExpr _ expr                             ->
    getEnsures methods stmts funArgs lv expr
  --ExcpExpr {excpName :: Exception, excpmsg :: Maybe String}
  ExcpExpr _ _                                  -> [Nothing]
  --ReturnExpr {returnE :: Maybe Expression}
  ReturnExpr Nothing                            -> [Nothing]
